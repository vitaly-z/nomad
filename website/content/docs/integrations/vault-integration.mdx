...
<EnterpriseAlert inline/>Nomad Enterprise users who are using [Vault
Namespaces][] should note that you will need to pass the `-namespace` flag for
each of the commands below. Each Vault namespace needs its own auth method,
role, and policies.
...
Nomad Enterprise allows jobs to use multiple Vault Namespaces. There are a few
configuration settings to consider when using this functionality.
